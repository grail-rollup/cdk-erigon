...
func (s *L1Syncer) GetBtcTxChan() chan string {
	return s.btcTxChan
}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				if latestBtcL1Block > lastCheckedBtcBlock && lastCheckedBtcBlock >= 0 { // TODO remove >=0 check
					s.isDownloading.Store(true)
